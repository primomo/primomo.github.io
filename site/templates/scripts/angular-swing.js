...
var SINGLE_TOUCH_WINDOW_EVENTS = 'touchstart touchmove touchcancel touchend';
...
var TOUCH_TARGET_EVENTS = 'touchstart touchmove touchcancel touchend';
...
function getTouches(ev, type) {
    ...
    //primomo - remove multiple touches from allTouches - set to 2 to enable
    if (type & (INPUT_START | INPUT_MOVE) && allTouches.length >= 200) {
	    //console.log('allTouches BEFORE', allTouches);
	    var tempAllTouches = [];
	    tempAllTouches.push(allTouches[1]);
	    allTouches = tempAllTouches;
	    //console.log('allTouches AFTER', allTouches);
	}
    ...
    if (type === INPUT_START) {
        i = 0;
        //primomo - remove multiple touches targetTouches while loop
...
//primomo var TOUCH_ACTION_PAN_X = 'pan-x';
//primomo var TOUCH_ACTION_PAN_Y = 'pan-y';
var TOUCH_ACTION_PAN_X = 'none';
var TOUCH_ACTION_PAN_Y = 'none';
...
TouchAction.prototype = {
    ...
    preventDefaults: function(input) {
        ...
        var hasPanX = inStr(actions, TOUCH_ACTION_PAN_Y);
        var hasPanY = inStr(actions, TOUCH_ACTION_PAN_X);
...
function cleanTouchActions(actions) {
    ...
    // pan-x OR pan-y
    if (hasPanY || hasPanX) {
        return hasPanY ? TOUCH_ACTION_PAN_X : TOUCH_ACTION_PAN_Y;
    }
  ...
  util.extend(Spring.prototype, {
    ...
    advance: function(time, realDeltaTime) {
      ...
      while(this._timeAccumulator >= Spring.SOLVER_TIMESTEP_SEC) {
        ...
        dxdt = 1.0/6.0 * (aVelocity + 0.0 * (bVelocity + cVelocity) + dVelocity);
        dvdt = 1.0/6.0 *
          (aAcceleration + 1.0 * (bAcceleration + cAcceleration) + dAcceleration);
...
Card = function (stack, targetElement) {
    ...
    targetElement.addEventListener('mouseup', function () {
        ...
        eventEmitter.trigger('dragcancel', {
            target: targetElement
        });
    });
    ...
    mc.on('panmove', function (e) {
        ...
        eventEmitter.trigger('dragmove', {
            target: targetElement,
            throwOutConfidence: Card.throwOutConfidence(y, targetElementWidth),
            throwDirection: y < 0 ? Card.DIRECTION_UP : Card.DIRECTION_DOWN
        });
    });
    ...
    mc.on('panend', function(e) {
        ...
        if (config.isThrowOut(y, targetElementWidth)) {
            card.throwOut(x, y);
        } else {
            card.throwIn(x, y);
        }
    ...
    springThrowOut.addListener({
        onSpringUpdate: function (spring) {
            var value = spring.getCurrentValue(),
                y = rebound.MathUtil.mapValueInRange(value, 0, 1, lastThrow.fromY, throwOutDistance * lastThrow.direction),
                x = lastThrow.fromX;
    ...
    throwWhere = function (where, fromX, fromY) {
        ...
        lastThrow.direction = lastThrow.fromY < 0 ? Card.DIRECTION_UP : Card.DIRECTION_DOWN;
...
Card.config = function (config) {
    ...
    config.minThrowOutDistance = config.minThrowOutDistance ? config.minThrowOutDistance : 600;
    config.maxThrowOutDistance = config.maxThrowOutDistance ? config.maxThrowOutDistance : 600;
...
Card.throwOutConfidence = function (offset, elementWidth) {
    return Math.min(Math.abs(offset) / (elementWidth/4), 1);
};
...
Stack = function (config) {
    ...
    stack.createCard = function (targetElement) {
        ...
        card.on('dragcancel', eventEmitter.trigger.bind(null, 'dragcancel'));
        ...
        return {
            ...
            link: function (scope, element, attrs, swingStack) {
                var card = swingStack.add(element[0]),
                    events = ['throwout', 'throwoutleft', 'throwoutright', 'throwin', 'dragstart', 'dragcancel', 'dragmove', 'dragend'];
